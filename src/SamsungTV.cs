...
/// <summary>
/// Manages a WebSocket client to a TV and allows sending requests to it
/// </summary>
public class TV : IDisposable
{
    ...
    public async Task<bool> IsOn(int delay = 0)
    {
        ...
        try
        {
            ...
            var json = Newtonsoft.Json.Linq.JObject.Parse(answer);
            string? name = (json?["name"] ?? json?["device"]?["name"])?.ToString();
            if (name != null)
            {
                Name = name;
            }
    ...
    /// </summary>
    /// <param name="key">A key from the list <see cref="SamsungKeys"/></param>
    ...
    private async Task ReadWebSocketLoop()
    {
        try
        {
            var buffer = new ArraySegment<byte>(new byte[1024]);
    ...
    private IPEndPoint CreateWolEndpoint()
    {
        ...
        return new IPEndPoint(new IPAddress(ipAddrBytes), _settings.Port);
    }
    ...
    private void Log(string msg)
    {
        if (_settings.Debug)
        {
            System.Diagnostics.Debug.WriteLine($"[TV] {msg}\n");
        }
    }
}
...
public class Settings
{
    ...
    public override string ToString() => 
        $@"AppName: {AppName}, Token: {Token}, IpAddr: {IpAddr}, MacAddr: {MacAddr}, Port: {Port}, Debug: {Debug}";
}

/// <summary>
/// Available keys to send to the connected TV
/// </summary>
...
/// <summary>
/// A command to send to the connected TV
/// </summary>
